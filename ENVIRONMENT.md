...
| `DATABASE_URL` | Postgres connection string, e.g. `postgresql://user:pass@host:5432/db?schema=public`. Unlike `REDIS_URL`/`RESEND_API_KEY` below, this one can't be a placeholder — the Prisma client connects lazily (see ARCHITECTURE.md), but essentially every page and action queries the database on the very first request, so an unreachable or fake value (the `.env.example` placeholder host included) fails immediately, not just for some later action. This is the one the *running app* uses — safe (in fact preferred) to point at a connection pooler (e.g. Supabase's pgbouncer). |
| `DIRECT_URL` | Optional. A **non-pooled** Postgres connection string, read only by `packages/db/prisma.config.ts` for the Prisma CLI (`migrate`/`studio`/`db seed`) — never by the running app. `migrate deploy` holds an advisory lock for the duration of the command, which a transaction-mode pooler (Supabase's pgbouncer on port 6543) doesn't support: every statement can land on a different underlying connection, so the lock silently never resolves and the command just hangs rather than erroring. Leave unset when `DATABASE_URL` is already a direct connection (e.g. local Docker Compose Postgres); set it when `DATABASE_URL` is pooled (Supabase: the same connection string with port `5432` and no `pgbouncer=true` — typically Supabase's own `POSTGRES_URL_NON_POOLING`). |
